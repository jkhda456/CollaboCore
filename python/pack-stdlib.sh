...
# Not usable here (no Tk, no ensurepip/pip, no _curses/_sqlite3) or only bulk: drop them.
( cd "$LIB" && rm -rf test idlelib tkinter turtledemo turtle.py ensurepip venv pydoc_data \
    curses sqlite3 lib2to3 config-3.13-* site-packages/* )
...
for f in "$LIB"/_sysconfigdata_*.py; do
  sed -i -e "s#$HOSTPY#/usr/bin/python3#g" \
         ...
         -e "s# *-[IL]$P/zlib[^ '\"]*##g" \
         -e "s#$P/src/Python-3.13.14#/usr/src/python#g" \
...
"$HOSTPY" - "$LIB" "$STAGE/usr/lib/python313.zip" <<'PY'
