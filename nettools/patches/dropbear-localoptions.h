 ...
 * (distro/distro/dropbear/localoptions.h, with the collaboCore changes marked.)
 *
...
/* collaboCore: unlike distro, the client may ask for a password. The guest console is a real
 * controlling terminal now, so getpass() has a /dev/tty to prompt on; only the SSH_ASKPASS
 * helper, which fork()s, stays out. Without a terminal (agent exec) the prompt fails at once. */
