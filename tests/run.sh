...
bash "$ROOT/engine/build.sh" --tests | grep -E "^test result:" || { echo "FAIL: engine unit tests"; exit 1; }
...
if [[ "$MODE" == "--boot" || "$MODE" == "--all" ]]; then
  ...
  out="$(NET=mock bash "$ROOT/userspace/test-boot.sh" "$ROOT/userspace/out/initramfs.cpio" \
    "wget -qO- --header 'x-test: 1' --post-data 'ping' http://api.example.test/v1/x")"
  grep -q "^\[host-fetch\] POST https://api.example.test/v1/x" <<<"$out" || { echo "$out" | tail -20; echo "FAIL: host did not see the request"; exit 1; }
  grep -q "mock response for https://api.example.test/v1/x" <<<"$out" || { echo "FAIL: guest did not get the response"; exit 1; }
  echo "guest -> host -> guest round trip ok (network device path)"

  echo "== guest boot: hfetch over vsock (request-level API, fake fetch)"
  out="$(API=mock bash "$ROOT/userspace/test-boot.sh" "$ROOT/userspace/out/initramfs.cpio" \
  ...
  grep -q '"x-api-key":"K"' <<<"$out" || { echo "$out" | tail -25; echo "FAIL: allowed headers/body did not reach fetch"; exit 1; }
  grep -q 'header-not-allowed: request header "cookie" is not allowed' <<<"$out" || { echo "FAIL: cookie was not refused"; exit 1; }
  grep -q '^rc=2' <<<"$out" || { echo "FAIL: refused request should exit 2"; exit 1; }
  grep -q '^rc=22' <<<"$out" || { echo "FAIL: -f on 404 should exit 22"; exit 1; }
  ...
  WORK=1 WORK_STATE="$state" WORK_OUT=/dev/null bash "$ROOT/userspace/test-boot.sh" "$ROOT/userspace/out/initramfs.cpio" \
    "mkdir -p proj && echo 'saved in visit one' > proj/note.txt" >/dev/null
  out="$(WORK=1 WORK_STATE="$state" WORK_OUT=/dev/null bash "$ROOT/userspace/test-boot.sh" "$ROOT/userspace/out/initramfs.cpio" "cat /work/proj/note.txt")"
  grep -q '^saved in visit one$' <<<"${out//$'\r'/}" || { echo "$out" | tail -15; echo "FAIL: workspace was not restored"; exit 1; }
  echo "restored after reload ok"

  echo "== guest boot: a full workspace refuses writes with ENOSPC and stays within its limit"
  out="$(WORK=1 WORK_MAX_BYTES=1000000 WORK_OUT=/dev/null bash "$ROOT/userspace/test-boot.sh" "$ROOT/userspace/out/initramfs.cpio" \
    "dd if=/dev/zero of=big bs=1024 count=2048 2>&1 | head -1")"
  grep -q "No space left on device" <<<"$out" || { echo "$out" | tail -15; echo "FAIL: no ENOSPC"; exit 1; }
  ...
  if [[ -f "$ROOT/python/out/python.cpio" && -f "$ROOT/python/out/test.cpio" ]]; then
    ...
    out="$(WORK=1 WORK_OUT=/dev/null EXTRA_CPIO="$PYCPIO" TEST_TIMEOUT_MS=400000 bash "$ROOT/userspace/test-boot.sh" \
    ...
    grep -q "^smoke-rc=0" <<<"$out" || { echo "$out" | tail -25; echo "FAIL: python smoke test"; exit 1; }

    echo "== guest boot: the standard library's extension modules, pydantic/jiter (Rust, built in), openai"
    out="$(WORK=1 WORK_OUT=/dev/null EXTRA_CPIO="$PYCPIO" TEST_TIMEOUT_MS=600000 bash "$ROOT/userspace/test-boot.sh" \
      "$ROOT/userspace/out/initramfs.cpio" "python3 /usr/share/tests/stdlib.py; echo stdlib-rc=\$?" | tr -d '\r')"
    echo "$out" | grep -E "^(FAIL|STDLIB:)"
    grep -q "^stdlib-rc=0" <<<"$out" || { echo "$out" | tail -25; echo "FAIL: python stdlib tests"; exit 1; }

    echo "== guest boot: python 'collabo_core' client against the host request bridge (fake fetch)"
    out="$(API=mock EXTRA_CPIO="$PYCPIO" TEST_TIMEOUT_MS=300000 bash "$ROOT/userspace/test-boot.sh" \
      "$ROOT/userspace/out/initramfs.cpio" "python3 /usr/share/tests/hostapi.py; echo hostapi-rc=\$?" | tr -d '\r')"
    echo "$out" | grep -E "^(FAIL|HOSTAPI:)"
    grep -q "^hostapi-rc=0" <<<"$out" || { echo "$out" | tail -25; echo "FAIL: collabo_core tests"; exit 1; }
  else
    echo "(python not built: skipping the Python guest tests; run: python3 build.py python)"
  fi
fi
if [[ "$MODE" == "--all" ]]; then
